...
document.getElementById("search-btn")?.addEventListener("click", () => {
  const inputCity = (document.getElementById("city-input") as HTMLInputElement).value;
  if (inputCity) {
    city = inputCity;
    getWeather(city);
    getForecast(city);
...
function getWeather(city: string) {
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      (document.getElementById("temp") as HTMLElement).innerText = `${temp}°C`;
      (document.getElementById("city") as HTMLElement).innerText = cityName;
      (document.getElementById("humidity") as HTMLElement).innerText = `습도  ${humidity}%`;
      (document.getElementById("wind") as HTMLElement).innerText = `풍속  ${windSpeed} m/s`;
      (document.getElementById("rain") as HTMLElement).innerText = `강수  ${rain}%`;
    })
...
// 시간대별 날씨 출력 (오늘 기준)
function getHourlyForecast(city: string) {
  const url = `https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=${API_KEY}&units=metric&lang=kr`;
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      const hourlyToday = data.list.filter((item: any) => {
        const itemDate = new Date(item.dt * 1000);
        return itemDate.getDate() === today;
      });

      const hourlyContainer = document.getElementById("hourly") as HTMLElement;
      hourlyContainer.innerHTML = "";

      hourlyToday.forEach((item: any) => {
